  ...
  function () {
    ...
    _createClass(WindowDatePicker, [{
      ...
      value: function setSecond(second) {
        ...
        if (second <= -1) {
          second = 59;
          this.setMinutes(this.value.minute - 1, true, false);
        } else if (second >= 60) {
          second = 0;
          this.setMinutes(this.value.second + 1, true, false);
        }

        this.value.second = second;

        if (input || !input && _second != this.value.second) {
          this.els.secondInput.value = Util.renderNumber(second);
        }
